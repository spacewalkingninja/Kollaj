        ...
        function identify(arr)
        {
          ...
          function actionCall(obj)
          {
            ...
            if (typeof obj.thePpzDatReFollingU != "undefined")
            {
              ...
              for (i in arr)
              {
                ...
                $("#usrFollowResImg"+i).css({"background-image":"url('http://192.168.1.95/kollaj/uploads/"+arr[i].usrAvatar+"')"});
            ...
            if(typeof obj.youCanBother != "undefined")
            {
              ...
              for (i in arr)
              {
                ...
                "</div></div>");
                $("#cropMRes"+i).css({"background-image":"url('http://192.168.1.95/kollaj/uploads/"+arr[i].avatar+"')"});
              }
          ...
          if (typeof obj.notifRes != "undefined")
          {
            ...
            for (i in arr)
            {
              ...
              "</p> </div>")
              $("#notifProfileP"+i).css({"background-image":'url("http://192.168.1.95/kollaj/uploads/'+arr[i].notifImage+'")'});
              ...
              if (hist=="feed")
              {
                ...
                $appendable.html("");
                  ...
                  for (i in arr)
                  {
                    ...
                    $("#usrVibedPostResImg"+i).css({"background-image":"url('http://192.168.1.95/kollaj/uploads/"+arr[i].usrAvatar+"')"});
...
*/
        ...
        var saveData = $.ajax({
              type: "POST",
              url: "http://192.168.1.95/kollaj/index.php",
        ...
        function dbValEmail (email)
        {
          ...
          identify(arr)
          var myRes = window.localStorage.getItem("emailPass");
          if (myRes=="pass") {return true} else {return false}
        }
        // checkUp userName @DB
        function dbValUname (uname)
        {
          ...
          identify(arr)
          var myRes = window.localStorage.getItem("unamePass");
          if (myRes=="pass") {return true} else {return false}
        }
        //emailPass
        ...
        $("#registerButton").click(function(){
          ...
          function sendMyStuff (uname, email, password )
          {
            ...
            identify(arr);
            var myRes = window.localStorage.getItem("registrationQuery");
            if (myRes=="pass") {return true} else {return false}
          }

          if(emailPass==1&&userPass==1&&pwPass==1 && $('#tosCheck').is(':checked') )
          {
            //Here we register

            sendMyStuff ($("#registerName").val(), $("#registerEmail").val(), $("#registerPassword").val() )
...
// IF(menuItemState == hidden) {#MenuItem.displayToggle}
        ...
        $("#sendPicLink").click(function(){
          ...
          if(profileState == 1) {$("#profile").css({"display":"none"})};
          ...
          // first we start loading pre-existing images so they are in the back
            ...
            var getProfile = $.ajax({
                  type: "POST",
                  url: "http://192.168.1.95/kollaj/index.php?seekProfile="+window.localStorage.getItem("loggedAs"),
                  data: JSON.stringify(arr),
                  dataType: "json",
                  contentType: 'application/json',
                  success: function(resultData){
...
//      	      touches = touches - 1;
          ...
          uploadPicDesc = $('#uploadPicDesc').val();
              ...
              function uploadPhoto(imgH, imgW, imgX, imgY, imageURI, mask1, mask2, scale, angle, tx, ty, nextPager) {
                  ...
                  ft.upload(imageURI, "http://192.168.1.95/kollaj/index.php", win, fail, options);
              }
          ...
          //          alert("Sent = " + r.bytesSent.toString()+"\n");
                  ...
                  if (the400kwh > 0)
                  {
                    console.log("ok, so we got to ad display");
                   //admob.isInterstitialReady(function(isReady){
                    //   if(isReady){
                    //     console.log("ad should display");
...
window.localStorage.setItem('history', 'feed');
    console.log("alo!");
      ...
      let dragEl = document.getElementById('picker')
      let ulEl = document.getElementById('mood-list')
      let ulHeight = ulEl.offsetHeight
      let liEls = ulEl.querySelectorAll('li')
      let liCount = liEls.length
      let liHeight = ulHeight / liCount
      let currentActive = 9;
      let distance = 1;
      let position = ulHeight;
      let velocity = 5;
      let friction = 0.8;
      let isDragging = false;
      let dragPositionY = position;
      let targetBound = 15;
      let attractionStrength = 0.05;
      let topBound = 0;
      let bottomBound = ulHeight - liHeight;
      ...
      $("#setMyAvatar").on('touchend mouseup', function(event) {
        ...
        return false;
            ...
            $("#myEmailSaver").on('touchend mouseup', function(event) {
              ...
              $("#callProfAction").click(function(){
                if(window.localStora.getItem("modal") == 1)
                {
                  $modal = $('.modal-frame');
                  $overlay = $('.modal-overlay');
                  window.localStorage.setItem("modal", 0);
